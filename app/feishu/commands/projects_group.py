...
def _get_all_threads():
    ...
    thread_list = []
    while True:
        resp = _fs.api.message.list(container_id_type='chat',container_id=PROJECT_CHAT_ID,page_size=50,page_token=page_token)
...
def _get_all_chat_members(member_id_type: str = 'user_id'):
    ...
    page_token = None
    while True:
        resp = _fs.api.chat.get_members(chat_id=PROJECT_CHAT_ID, member_id_type=member_id_type,page_size=50,page_token=page_token)
        page_token = safe_get(resp,"data","page_token")
    ...
    for thread in thread_list:
        try:
            ...
            if inventory_name != None and inventory_name not in existed_inventory_name:
                    _fs.api.task.create_inventory(name=inventory_name, members=[{'id':user_id, 'role':'editor'} for user_id in chat_members_user_id_list])
...
def new_thread_in_project_group_callback(message: str):
    ...
    if inventory_name != None:
        resp = _fs.api.task.create_inventory(name=inventory_name, members=[{'id':user_id, 'role':'editor'} for user_id in chat_members_user_id_list])
        logger.info("add task inventory %s" % inventory_name)
